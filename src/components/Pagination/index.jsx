...
import { Pagination as BSPagination, Row, Col ,Dropdown } from 'react-bootstrap'
  ...
  const handlePageSize = (pageSize) => () => {
    ...
    return (
      <Row style={{ justifyContent: 'center' }}>
        ...
        <Col xs='auto' className='pr-1 pl-1'>
          <BSPagination>
              <BSPagination.Prev tag='button' onClick={handlePageClick('prev')}
                disabled={pagination.page === 1} />
            {pages.map((item, index) => (
              <BSPagination.Item key={index+item} active={index + 1 === pagination.page} onClick={handlePageClick(index + 1)}>
                  {index + 1}
              </BSPagination.Item>
            ))}
              <BSPagination.Next tag='button' onClick={handlePageClick('next')}
